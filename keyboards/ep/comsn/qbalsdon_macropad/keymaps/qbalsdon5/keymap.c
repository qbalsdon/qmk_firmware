...
bool encoder_update_user(uint8_t index, bool clockwise) {
    if (index == 0) { /* First encoder */
        if (clockwise) {
          oled_write_P(PSTR("clockwise: "), false);
          rgblight_setrgb_at(20, 20, 0, KEY_LED_0);
            tap_code(KC_PGDN);
        } else {
          oled_write_P(PSTR("anti-clockwise: "), false);
...
void keyboard_post_init_user(void) {
    ...
    oled_clear();
    rgblight_setrgb_at(0, 0, 0, KEY_LED_0);
    rgblight_setrgb_at(0, 0, 0, KEY_LED_1);
    rgblight_setrgb_at(0, 0, 0, KEY_LED_2);
}
